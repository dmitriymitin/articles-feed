...
import { articleTestData1, articleTestData2 } from "@/entities/Article/testing";

import { ArticleDetailsRecommendationsSchema } from '../../..';
import { fetchArticleRecommendations } from "../../services/fetchArticleRecommendations/fetchArticleRecommendations";

import { articleDetailsPageRecommendationsReducer, recommendationsAdapter } from './articleDetailsPageRecommendationsSlice';
...
describe('articleDetailsCommentsSlice.test', () => {
  test('test get article details comments pending', () => {
    ...
    expect(
      articleDetailsPageRecommendationsReducer(state as ArticleDetailsRecommendationsSchema, fetchArticleRecommendations.pending),
    ).toEqual({
      isLoading: true,
      error: undefined,
    });
  ...
  test('test get article details comments rejected', () => {
    ...
    expect(
      articleDetailsPageRecommendationsReducer(state as ArticleDetailsRecommendationsSchema, {
  ...
  test('test get article details comments fulfilled', () => {
    const state = recommendationsAdapter.getInitialState<ArticleDetailsRecommendationsSchema>(
      {
        isLoading: true,
        error: undefined,
        ids: ['2'],
